...
def _prune(archive_dir: str, stem: str, keep: int) -> None:
    """Delete the oldest archives beyond ``keep``.

    Ordered by mtime, not filename — same-second rotations get ``-N``
    collision suffixes that would sort lexically BEFORE their base name.
    """
    try:
        archives = sorted(
