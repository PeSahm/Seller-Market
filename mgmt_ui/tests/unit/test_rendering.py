...
def test_config_ini_empty_when_no_customers() -> None:
    ...
    # Header comment + the hot-reload sentinel (so a disarm-all config is a
    # COMPLETE file the bot trusts immediately). The original trading bot
    # iterates config.sections() which is [] for an empty body; no [DEFAULT]
    # block — that header was an early scaffolding decision that caused
    # parser surprises on the bot side.
    expected = (
    ...
    # configparser still sees zero sections (the sentinel is a comment).
    import configparser
