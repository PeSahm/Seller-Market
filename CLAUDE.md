...
**Deploy state now (canary ARMED, not yet fired) — bot on `f434e48`:**
- **Bot image** is now **`f434e48`** (= `6b25a56` rlc_price + the cache_warmup Exir fix below). Staged on 5.10.248.55 by digest (`ghcr-mirror.liara.ir/pesahm/seller-market@sha256:bc205c7c…`; the earlier `6b25a56` was `sha256:5e2c4673…`) → retagged `ghcr.io/…:latest`, revision verified. ghcr **was blocked** from PouyanIt → mirror-by-digest was required.
- **Mostafa's stack** (`83619dcd`, dir `…/89bb891e-…/`) recreated on `f434e48`, **healthy**. Both customers on it: ephoenix `ayandeh IRO1PNES0001 s1` (control) + exir `khobregan IRO1SROD0001 s1` (سرود, buy). config.ini carries `broker_family = exir` for khobregan. Scheduler: cache_warmup 08:20, run_trading 08:44.
...
4. Then **roll the other 6 stacks** to `f434e48` (they still run `902a3dd`): stage the image per host (PouyanIt already has it; Tebyan via mirror-by-digest), recreate each stack dir's compose, verify the running container's revision == `f434e48`.
