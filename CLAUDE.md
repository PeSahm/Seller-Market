...
- **No mgmt restart needed (vs the S31 strand)**: I ran every prep step via one-shot `docker exec -i …api… python -` processes — EACH has its OWN module-level SSH pool, so the connection that predated the `usermod -aG docker` was torn down with that process. The long-running **uvicorn** web process never connected to Tebyan6 before the group add, so its pool opens fresh (group-aware) on the operator's first deploy. (S31's strand happened because the operator's web-UI readiness probes connected `.5` BEFORE the group add, leaving a stale uvicorn-pool connection.)
- **Verified**: `test_connection` → `ok=True`, `base_dir_writable=True`, `docker_version=29.1.3`, clock skew +1s, `host_key_mismatch=False`; and a fresh non-sudo `docker ps` + `docker compose version` succeed for `bargozideh` (proves the docker group works on a new connection → dashboard deploys will work). **No stacks on Tebyan6 yet** — operator deploys/assigns via the dashboard.
- **Learnings**: (1) onboard a new host via `create_server` + `run_command` (password auth, TOFU) — no local paramiko/key. (2) `download.docker.com` GPG-path 403 on some Iranian providers → use Ubuntu `docker.io`+`docker-compose-v2`. (3) one-shot `docker exec python` processes each get their own SSH pool, so they DON'T leave a `usermod`-strand in the uvicorn pool (no restart needed) — unlike web-UI-driven probes.
