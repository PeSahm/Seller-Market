...
### Live-tested against the operator's account (CodeRabbit caught nothing here — the LIVE test did)
Before merge, ran the ACTUAL adapter code (bot + mgmt) against account `4580090306`: login VALID, **buyPower 31,224,500**, name مصطفی اسماعیلی / bourse اسمـ50113, sizing correct; created a real draft for `IRO1NKOL0001` (شكلر) → id `01KWBWKJHF75B5ANFPM7Z6QB3J` (1-share, NOT fired). This is what surfaced the same-login + ULID bugs. **RLC is flaky from Windows** — used tsetmc for the شكلر price (ceiling 20870) and a `price` override.

### CodeRabbit (15 findings) → 10 fixed, 5 declined
Fixed: proxy verify is credential-only (a valid login with 0 BP / no price = valid, not transient); 401/403 retry-once in mgmt `verify_credentials`+`get_holdings`; `admin` verify-isin degrades on a cache/DB blip (not 500); `run_mofid` defensive side-parse + **non-daemon** firer threads (no kill mid-POST); honor explicit-empty `config_section`; generic locust log; **redacted the live account id** from tests; spike OCR `trust_env=False`. Declined: `state=3` is the family-wide filled-feed convention (exir/onlineplus identical; `is_done` carries partial); `direct_sell._DIRECT.trust_env=False` already set (false positive); the remaining `mofid_spike` nits are on a throwaway script.

### State — MERGED + DEPLOYED FLEET-WIDE (squash `2c2a1dc`, PR #176)
- **mgmt** both instances (PouyanIt + ParsPack) on `2c2a1dc`, `/health=200`, **alembic `0023_seed_mofid`**, the `mofid` broker row seeded+enabled, **worker-leader still PouyanIt** (deployed ParsPack first to preserve it).
- **bot**: all **24/24** stacks across the 6 Tebyan hosts redeployed onto `2c2a1dc` (image staged by mirror — fresh, rev==SHA — then `redeploy_stack` ×24 via the api container, `warm_family_cache` first; 14:15 Tehran = off-market, 0 armed auto-sell = safe). Every running container revision-verified == `2c2a1dc`. ephoenix/exir/onlineplus byte-identical; Mofid is additive + **dormant** (no Mofid customer yet).
- **CANARY still OPERATOR-GATED** (a REAL bounded BUY at the 08:44 open). To run it: (1) add a `mofid` customer in the UI + Verify credentials, (2) the customer must HOLD/fund the account, (3) **render a `run_mofid` scheduler job @08:44** for that stack (the locust `run_trading` job EXCLUDES mofid — without `run_mofid`, nothing fires), (4) watch the open. Safety: `mofid_draft_count=1` (no over-buy), market-hours + operator-present.
- **Open follow-ups**: the **`run_mofid` scheduler job is NOT auto-rendered** for Mofid stacks yet (mgmt scheduler-defaults change, or the operator adds it manually) — REQUIRED for firing; confirm `createDateTime` format + GetOrders field shapes at the first executed order (mapper handles ISO+Jalali + ULID); the **1-share شكلر test draft `01KWBWKJHF75B5ANFPM7Z6QB3J`** is in Mostafa's easytrader drafts (deletable).

---

## Session 41 — "mofid missing from the broker dropdown" root-caused = mgmt running on a STALE failed-over SPARE DB; full no-data-loss recovery (failback to main)

Operator (the morning after the S40 Mofid deploy): *"I don't see mofid in the list of brokers to create a customer"* — persisting even in a fresh browser after clearing cache. After S40 verified the backend + DB all had mofid. The operator pasted the live `/admin/customers/new` page source: a **fresh page (fresh CSRF token) with CURRENT agents but the broker `<select>` ending at onlineplus/Hafez — NO mofid optgroup.** Pure-ops recovery (no code/migration), all on `5.10.248.55`/`45.139.10.192`.

### Root cause — the mgmt UI was serving a STALE auto-failed-over spare (and the two instances were SPLIT)
- **PouyanIt auto-failed-over to the warm spare DB on Jun 30 05:24** (a transient main-DB blip; S24 durable-failover). The `/var/lib/sm-mgmt/backups/FAILOVER_ACTIVE` marker was present; api logs: `db_failover: marker present at startup — rehydrating onto the SPARE` + `DB FAILOVER: shared sessionmaker rebound to the SPARE database` + `main reachable again — resync + restart to return`.
- **The S40 mgmt deploy (`compose up -d api`) RESTARTED the api → it saw the marker at startup → REHYDRATED onto the spare** (by design: "marker present ⇒ rehydrate to spare; NEVER auto-clear"). So a "successful deploy" silently kept serving the stale spare.
- **The `0023_seed_mofid` migration ran on the MAIN** (alembic uses a separate engine against `DATABASE_URL`, not the failover-aware sessionmaker) — but the **spare froze before 0023** (the backup cron SKIPS while the marker exists, so the spare never got the migration). → the spare has no mofid broker row → the live dropdown lacked it.
- **The two mgmt instances were SPLIT** (failover is per-instance): **PouyanIt → spare** (97 customers, no mofid), **ParsPack → main** (91 customers, has mofid, never failed over). The operator uses PouyanIt → saw the spare. Anyone on ParsPack would've seen mofid.
- **Divergence** (measured both DBs): MAIN `alembic=0023, customers=91, mofid=1`; SPARE `alembic=0022, customers=97, mofid=0`. The spare had **6 MORE customers** — the day's UI adds went to the spare, AND (since the worker-LEADER PouyanIt was on the spare) ALL ingest-worker writes (runs/trade_results/credential statuses) went to the spare. The main's only day-writes were my S40 `docker exec` redeploy audit/stack-status (fresh-process → main, regenerable). So **spare ⊇ main for all important data.**

### The diagnostic trap (the headline lesson)
**A `docker exec … python -c "active_db()"` / `list_enabled_grouped` probe runs in a FRESH process → no failover state → ALWAYS hits the MAIN** (returns mofid). The **running uvicorn** (full lifespan → ran the marker rehydration) was on the **SPARE** (no mofid). So every backend probe I ran said "mofid is there" while the live server served the opposite. **A fresh-process `active_db()` is MEANINGLESS for the running server's DB** — to know which DB the live uvicorn uses, check (a) the marker file, and (b) the uvicorn's OWN logs for the `rehydrating onto the SPARE`/`rebound to the SPARE` line, NOT a separate python process. The operator's pasted page source (fresh CSRF) was the real evidence; my ASGI probe (no lifespan → main) had falsely "confirmed" mofid.

### Full recovery — operator chose "keep everything" (no data loss + restore HA)
Since `spare ⊇ main` (after adding mofid to the spare), the clean path is **dump spare → restore main → fail back**:
1. **Make the spare current**: `INSERT` the mofid broker row (mirroring 0023's 5 cols — `id`/timestamps/`enabled`/`sort_order` have server defaults) + `UPDATE alembic_version='0023_seed_mofid'` on `sm-spare-pg`. (This alone would've shown mofid on the live spare — but I completed the failback.)
2. **Safety-backup the MAIN** first: `pg_dump` → `/var/lib/sm-mgmt/recovery/main_pre_recovery_<ts>.dump` (rollback insurance; the spare also stays intact as a fallback).
3. **Stop BOTH api** (quiesce — releases all DB connections; ~1–2 min downtime; the marker still present so the cron stays skipped → no interference).
4. **Dump spare → restore main** via the `sm-spare-pg` PG18 client (the S25 backup-cron mechanics in REVERSE): `pg_dump -Fc` the spare → `pg_restore --clean --if-exists --no-owner --no-privileges -h 87.107.164.154 -p 65444` into the main. **exit 0**; main verified `alembic=0023, customers=97, mofid=1`.
5. **Remove the FAILOVER_ACTIVE marker** on PouyanIt (the operator action the durable-failover design requires).
6. **Start PouyanIt first** (boots on the main, reacquires the worker-leader advisory lock → leader) **then ParsPack** (lock held → standby). Confirmed each on the main: new-session logs have NO rehydrate line, `main_advisory_locks=1` (single leader = PouyanIt), `spare_app_conns=0`.
7. **Resume the backup cron** (marker gone): ran it once → `restored=true`, manifest count=4 (HA Backups card un-frozen); spare re-synced to match the main (97 + mofid + 0023).
- **Verified end-to-end**: the ASGI probe of the real `/admin/customers/new` (now on the main) renders `<optgroup label="mofid"><option value="mofid">Mofid (Orbis)</option></optgroup>`. Both `/health=200`.

### Learnings (Session 41)
- **A fresh `docker exec python` process NEVER reflects the running uvicorn's failover state** — it always binds the main. Diagnose the live server's DB via the **marker file + the uvicorn's own rehydrate log line**, not a side process. (This is why S25/S38 "confirmed active_db=main via docker exec" can be a false negative for a failed-over instance.)
- **Restarting the api while `FAILOVER_ACTIVE` exists REHYDRATES onto the spare** (durable-failover by design). So a deploy/`compose up -d api` during an unnoticed failover SILENTLY keeps serving the stale spare. **After ANY mgmt restart, verify the running server is on the main** (marker absent + no rehydrate log + `advisory_locks` holder), not just `/health=200`.
- **Failover is PER-INSTANCE → the two mgmt instances can split** (one on spare, one on main) serving different data. And a **migration runs on the main (separate alembic engine) but the frozen spare never gets it** → the failed-over instance lacks the migration's effect (here: the mofid row).
- **When `spare ⊇ main` (the leader's workers + UI wrote to the spare during the outage), recover by making the spare current (apply the missed migration) → dump spare → restore main → fail back.** Safety-backup the main first; keep the spare intact as the fallback; the same PG18→PG18 `pg_dump`/`pg_restore --clean` mechanics as the S23 cutover / S25 backup cron (just reversed).
- **The marker is durable failover STATE, not a transient flag** — only the operator removes it (after resync). An unnoticed marker means the system has been on the spare (degraded HA, frozen backups) since the failover; surface it (the operator had been on the spare ~1 day without knowing). Candidate follow-up: an `/admin/ha` banner / alert when `on_spare` is true (the data exists — `build_ha_status` returns `on_spare`).

---

## Session 42 — Mofid FIRES: independent scheduler (#177) + LIVE firing settings (#178/#179); DB failed BACK to main; whole fleet rolled to `c2f92a7`

Continued S41. Root-caused why the operator's Mofid customer never fired, built an **independent Mofid firer** + a **live-config Settings card**, and completed the **DB failback** (both mgmt instances back on the main). PR map: **#177** `e567503` independent scheduler + idempotency latch · **#178** `c89401c` Mofid firing settings · **#179** `c2f92a7` run_time made instant. **Fleet rolled twice** (e567503→c89401c→`c2f92a7`).

### DB failback — the S41 recovery hadn't fully stuck (split-brain), fixed cleanly
- State on entry: the `FAILOVER_ACTIVE` marker had VANISHED (operator removed it) AND main/spare had CONVERGED to 98 customers (a reconcile happened) — BUT the **running PouyanIt api was STILL on the spare** (StartedAt 21:54, rehydrated at that startup; removing the marker does NOT auto-failback). **ParsPack was on the MAIN** (no marker at its 21:55 startup). So a genuine **split**: PouyanIt→spare, ParsPack→main, BOTH taking live writes.
- **The clobber trap**: marker gone ⇒ the backup cron RESUMES (`main→spare`) ⇒ would clobber the live spare (PouyanIt's DB) on the next UI write. Paused the PouyanIt cron immediately to protect it.
- **Safe no-dump failback**: verified the IMPORTANT data is **md5-IDENTICAL between main and spare** — `md5(string_agg(id ORDER BY id))` of customers / trade_instructions / brokers.code / agent_stacks all matched (98/84/19/24). So NO split-brain on what matters (only operational tables — runs/audit — differ, and those re-ingest). → the failback needs no dump (the main already has everything that matters).
- **Executed (deterministic transfer, keep PouyanIt leader)**: pause PouyanIt cron → safety-backup the main (`pg_dump` → `/var/lib/sm-mgmt/recovery/main_pre_failback.dump`) → **stop ParsPack api** (releases the main's leader advisory lock) → **restart PouyanIt api** (marker gone → boots on main → acquires the free lock → LEADER) → **start ParsPack api** (lock held → standby) → resume cron. Verified: `leader_lock=5.10.248.55` (PouyanIt, `n=1`), both instances heartbeating to the main (`mgmt_instances.last_seen` fresh), no data loss.

### Mofid customer "never fired" — THREE compounding gaps
Customer مصطفی ایزی تریدر (`4580090306`, Mostafa's `.180` stack `fc83bd8c`):
1. **0 trade instructions** at first (operator then added 2 at 01:42: `IRO1DPAK0001`+`IRO1DZAH0001`; later removed one → only `IRO1DPAK0001` remains — that's intended). A 0-instruction customer renders no config section (S20).
2. **No `run_mofid` scheduler job** anywhere in the fleet (Mofid is excluded from the locust `run_trading`).
3. **THE architectural blocker: the bot `JobScheduler` runs jobs SEQUENTIALLY** (`execute_job` → blocking `subprocess.run`; the loop runs jobs one at a time). On a **mixed** stack (this one has karamad ephoenix via locust AND Mofid), `run_trading` (08:44, locust) and `run_mofid` can't both be live at the 08:45 open — whichever runs first blocks the other past its window.
- **`config.ini` looked stale vs the DB but actually MATCHED a just-changed DB**: config had 1 mofid section (`t651b79f9`) while an earlier query showed 2 (`t945eef45`+`t1c9a301b`) — because the operator **re-did the instructions at 02:48** (deleted the 2, added 1). DB==config==1. **Check the row timestamps before assuming config staleness.**

### The independent Mofid scheduler (#177 `e567503`) — operator: "run it independent, don't touch the shared scheduler"
- A **SECOND, independent `JobScheduler` thread** dedicated to `run_mofid`, launched by `bot_entrypoint._start_mofid_scheduler` **ONLY when `config.ini` has Mofid BUY sections** (gated via `run_mofid.mofid_buy_targets` → byte-identical no-op on non-Mofid stacks). Fires `run_mofid.py` at the open **CONCURRENT with (never blocking)** `run_trading` — different threads/subprocesses, no shared mutable state (distinct per-run marker UUIDs, disjoint job names). Reuses the base JobScheduler's marker/timeout/log machinery.
- `run_mofid.py` refactored: extracted `mofid_buy_targets()` (the shared gate) + moved logging setup into `main()` (`_setup_logging`) so **importing run_mofid is side-effect-free** (the gate calls it at bot startup).
- **Adversarial review found a MEDIUM double-fire** (no cross-restart latch, unlike auto-sell's DayState): a container restart inside the scheduler's 120s window could re-create drafts and DOUBLE-BUY. **Fix**: a per-`(account,isin)`-per-day marker in `run_results/` (bind-mounted → survives restart). `fire_section` **skips a latched section** and **latches BEFORE evaluating success** (a crash after the broker accepted can't re-fire); a pre-fire login/draft failure is NOT latched (can retry). Pruned after 7 days.

### Mofid firing SETTINGS (#178 `c89401c`) + run_time made INSTANT (#179 `c2f92a7`)
- Added a **"Mofid / Orbis firing"** card to Admin → Settings via the existing `bot_rt_*` `[runtime]` mechanism (DEFAULTS + `SettingsUpdate` schema + `admin_settings_save` route + template + tests). Save → `[runtime]` re-rendered + pushed fleet-wide → the bot reads it. **draft_count** (1-50, read by `mofid_adapter.prepare_order` at fire time) + **fire time / deadline** (`mofid_firer.window_config` at fire time) apply on the **next open, no redeploy**.
- Operator: **"all settings should apply instantly."** draft_count + window WERE instant (fire-time reads); **run_time** (draft-creation start) was the exception — read once at bot STARTUP to build the Mofid scheduler → needed a redeploy.
- **Fix (#179)**: the Mofid scheduler now reads `mofid_run_time` from `[runtime]` **LIVE each loop** — a local `_MofidScheduler(JobScheduler)` subclass overriding `load_config()` → `_mofid_schedule_config()` (the base JobScheduler re-reads its config every iteration). **No static `mofid_scheduler_config.json`** anymore. So ALL 4 Mofid knobs (drafts · draft-creation start · fire time · fire deadline) apply on the next open with no redeploy. A mid-day change can't double-fire — the per-`(account,isin)` day latch guards it.
- **Draft timing (confirmed)**: `run_mofid` starts at `mofid_run_time` (default 08:44:00) → logs in + **creates the N drafts** (~1 min before the open, off the hot path) → `mofid_firer` waits then **batch-sends** in the window (08:44:58.450→08:45:00.900 server time, the operator's "08:44:59"). Drafts are DAY-valid so pre-creating a minute early is fine.

### Fleet rolled to the latest engine (`c2f92a7`) — refined runbook
Rolled all **24 stacks / 6 hosts** twice (e567503→c89401c→c2f92a7). **Runbook**: temporarily **DISABLE autobalance** (`enable_autobalance=false` — an engine update must NOT reshuffle customers) → **stage the image on all hosts in PARALLEL** (`run_command` + mirror-pull-and-verify `revision==<sha>`, retag `:latest`) → **redeploy all stacks SEQUENTIALLY** via `redeploy_stack` run **DETACHED in the api container** (`nohup python -u … > /tmp/log &`) + poll the log (survives SSH drops; ~10 min for 24), `warm_family_cache` first → **re-enable autobalance** → **verify every running container's `revision` label** ("up is not proof"). Both mgmt deploys used **ParsPack (standby) first, PouyanIt (leader) last** so PouyanIt re-acquires the lock on its restart (the standby doesn't re-elect at runtime).

### Post-deploy: the 08:44 canary DIDN'T fire — run_mofid subprocess-timeout bug (#180 `8e0df21`, fleet re-rolled)
Next morning the Mofid order didn't fire. The operator had set `mofid_run_time=08:30` (via the new instant setting) — **15 min before** the 08:44:58 window. `run_mofid` fired at 08:30, logged in, created 10 drafts, ARMED the window... then **the scheduler's fixed 600s `subprocess.run` timeout KILLED it at 08:40** (`Job 'run_mofid' timed out after 600 seconds`), ~5 min BEFORE the window → `run_mofid: done — 0/1 fired`, no batch sent. (The karamad/ephoenix locust orders on the SAME stack DID fire at 08:45 — only Mofid was killed.) **Both timeouts assumed run_time ≈ window−1min**: `scheduler._compute_job_timeout` returns the 600s default for `run_mofid.py`, AND `run_mofid.main()`'s `_JOIN_TIMEOUT_S=480s` join.
- **Fix (#180)**: new `mofid_firer.window_end_local_ms()`; `_compute_job_timeout` special-cases `run_mofid.py` → cap at `window_end_local + grace` (1800s fallback); `run_mofid.main()` joins until `window_end + 60s`. So ANY `run_time` earlier than the window survives to fire. **Rolled fleet-wide → all 25 stacks on `8e0df21`** (the fleet grew 24→25 mid-session). Verified live: the timeout computes ~600 AFTER the window (past → floored) but ~1080 at 08:30 BEFORE it (unit-tested with a future window). Mofid scheduler re-armed post-roll.
- **LESSON: a scheduled job that WAITS in-process until a LATER window needs its subprocess AND join timeouts sized to that window, not a fixed default.** And this is BOT code (the timeout), so it needs a bot deploy to fix — unlike the `[runtime]` settings. **Tell the operator run_time only needs ~1 min of headroom** (login+drafts take ~1s); a far-early run_time works now but uses a staler price.

### FIRST LIVE MOFID ORDER — fired successfully mid-session (operator-requested live test) ✓
Operator: "run trade for IRO1DPAK0001 and test it now in market live." Since the 08:44 window was long past, replicated `run_mofid.fire_section` MINUS the window wait: `get_adapter` → `adapter.prepare_order(isin, side=1, config_section)` (login → BP → RLC ceiling → create N drafts → batch body) → **`direct_sell.send_prepared_order(prepared)`** (the SAME POST run_mofid uses, fired immediately). Result: **HTTP 200**, batchCreate returned `["01KWE3PGQZX6HWT8RZAHPE13KW"]`, `order_fire_log.emit_order_fire(...)` for the mgmt date-based reconcile.
- **Order confirmed LIVE in the book** via `adapter._authed_get("/core/api/order", adapter._session())`: `{id:"1121FeMq3wD.1SUn", symbolIsin:IRO1DPAK0001, symbolName:دارو, price:4820, quantity:6452, executedQuantity:0, side:0(buy), orderStateStr:"OnBoard", trades:[]}`. **`OnBoard` = accepted/active in the exchange order book** (queued at the ceiling; a limit-up buy-queue stock → 0 filled until sellers appear). **The batch of 10 drafts produced exactly ONE order** (`orders` list length 1) — the bounded no-over-buy behavior held live.
- **So the whole Mofid pipeline is proven end-to-end LIVE**: login (OAuth+captcha) → buying power → RLC ceiling price → N drafts → batchCreate → resting order. Tomorrow's scheduled 08:44 fires this automatically (with the #180 timeout fix). The operator left the test order queued (order `1121FeMq3wD.1SUn`).
- **REUSABLE — live-test a window-gated order OUTSIDE its window**: `prepare_order` + `direct_sell.send_prepared_order` (no window wait); check state via the bot adapter's `_authed_get("/core/api/order", _session())` (`orderStateStr`/`executedQuantity`/`trades`). The batchCreate response is a ULID list; the RESTING order has a different broker `id`.

### Learnings (Session 42)
- **A fresh `docker exec python` process ALWAYS binds the main** (no failover state) — `active_db()`/`list_enabled_grouped` from a side process is MEANINGLESS for the RUNNING uvicorn's DB. Diagnose the live server via the **marker file + the uvicorn's OWN startup logs** (the `rehydrating onto the SPARE` line) + the **advisory-lock `client_addr`**, not a side probe. (S41's headline, re-confirmed.)
- **Failover is per-instance → a split is real** (PouyanIt-spare + ParsPack-main both writing). **md5 of key-table ids** is the fast way to prove the important data is identical → a **no-dump failback** is safe (the operational-table divergence re-ingests).
- **Never dump one DB over the other during a split without checking both directions** — a one-way `pg_restore --clean` loses the OTHER side's writes. Compare first.
- **The bot `JobScheduler` is SEQUENTIAL** (blocking `subprocess.run`) — two jobs that both need the 08:45 open can't share it. The fix for a family that can't ride locust (Mofid's 1500/hr cap) is a **SECOND independent JobScheduler thread** (concurrent, gated on that family's sections).
- **`JobScheduler.load_config()` is re-read every loop** → a subclass overriding it makes any config value (here `mofid_run_time`) **live/instant** without a file or a redeploy. Clean way to make a scheduled time DB-tunable.
- **Cross-restart idempotency needs an on-disk latch** (`run_results/` marker, survives restart), latched BEFORE evaluating success so a crash-after-broker-accept can't re-fire; a pre-fire failure stays un-latched (retryable). Same shape as auto-sell's DayState.
- **`config.ini` can look stale vs the DB but actually match a JUST-CHANGED DB** — check `created_at` on the rows before assuming a config-push bug (the operator had re-created the instruction at 02:48).
- **`docker exec … python` roll pattern**: run the redeploy loop DETACHED in the api container (`nohup … &` → `/tmp/log`) + poll — an SSH blip can't interrupt a 10-min fleet roll.
- **CodeRabbit "pending" doesn't block a green-CI merge** — merge `--admin --squash` once `test`+`mgmt-ui-test` pass; the branch-protection rule needs `--admin` here.

### Deploy state (END S42 — all verified live, 2026-07-01 ~09:10 Tehran)
- **Bot: all 25 stacks on `8e0df21`** (independent Mofid scheduler + idempotency latch + LIVE run_time + the run_mofid timeout fix #180). Fleet = **25 stacks / 6 Tebyan hosts**: `.177`×5, `.180`×1, `.189`×5, `.246`×6, `.39`×4, `.5`×4 (grew 24→25). `armed_auto_sell=0`. PouyanIt + ParsPack = mgmt/OCR/DB-spare/market-data only (no bot stacks).
- **Mgmt: both instances on `c2f92a7`** (PouyanIt LEADER + ParsPack standby), on the **external Windows PG18 main** (`87.107.164.154:65444`), alembic **`0023_seed_mofid`**, single worker-leader. The 4-field "Mofid / Orbis firing" Settings card is live (all instant). (mgmt unchanged by #180 — bot-only.)
- **Mofid canary — FIRED LIVE ✓**: `.180` Mostafa stack (`mofid_run_time=08:30`, `draft_count=10`), scheduler active. The scheduled 08:44 attempt was killed by the #180 timeout bug (now fixed); then **manually test-fired `IRO1DPAK0001` LIVE at 09:21** → HTTP 200, order placed & resting in the book (id `1121FeMq3wD.1SUn`, دارو, BUY 6452 @ 4820 ceiling, `OnBoard`/queued, bounded to 1 order). Pipeline proven end-to-end; the scheduled 08:44 now fires it automatically. Test order left queued.

### Open follow-ups (Session 42)
| # | Title | Why |
|---|---|---|
| — | ~~Watch the 08:44 open for the Mofid canary~~ | **DONE** — the pipeline is proven live (manual fire 09:21, order resting in the book). The scheduled 08:44 firing is now automatic (#180); just confirm it fires unattended once. |
| — | 6 stale `running` runs (Jun 29) | Leftover stuck rows from the DB-split period (no terminal marker) — finalize so `/admin/runs` is tidy. |
| — | First-Mofid-customer gate needs a redeploy | The scheduler-launch gate runs at container start, so adding the FIRST Mofid customer to a running stack still needs one redeploy to launch the thread (the SETTINGS are instant; the launch isn't). Could make the gate dynamic if it bites. |
| — | `/admin/ha` on_spare banner/alert | Carried from S41 — would have surfaced the day-long failover immediately (`build_ha_status` already returns `on_spare`). |
| — | Fernet `key.part2` missing | Every mgmt op logs the DEV-MODE part2 warning (carried from S20). |
