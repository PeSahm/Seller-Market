...
### Deploy state (END OF SESSION)
- **All 3 PRs MERGED to `main`** (`f1ac9bd` = #174 on top of `66f7800` #173 on top of `38fbcbc` #172). **NOTHING DEPLOYED YET** — operator deploys the mgmt image (migration 0022 runs on startup) + does the bot canary. The bot firing code is **dormant** until a stack is redeployed onto the new bot image AND a Hafez/OnlinePlus customer is armed.
- mgmt instances were on `cc8ac7f` (an operator asyncpg-cache fix committed on top of #172 mid-session); main is now `f1ac9bd`.
