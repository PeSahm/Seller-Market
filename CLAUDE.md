...
### Deploy state (END S42 — all verified live, 2026-07-01 ~04:00 Tehran)
- **Bot: all 24 stacks on `c2f92a7`** (independent Mofid scheduler + idempotency latch + LIVE run_time). Fleet = 24 stacks / **6 Tebyan hosts**: `.177`×5, `.180`×1, `.189`×5, `.246`×6, `.39`×4, `.5`×3. `armed_auto_sell=0`. PouyanIt + ParsPack = mgmt/OCR/DB-spare/market-data only (no bot stacks).
- **Mgmt: both instances on `c2f92a7`** (PouyanIt LEADER + ParsPack standby), on the **external Windows PG18 main** (`87.107.164.154:65444`), alembic **`0023_seed_mofid`**, single worker-leader. The 4-field "Mofid / Orbis firing" Settings card is live (all instant).
- **Mofid canary**: `.180` Mostafa stack, armed, `run_mofid` scheduler active → fires **`IRO1DPAK0001` (دپارس)** at the next 08:44 open (NOT yet fired — set up ~03:00; watch the open).
