...
- **Mofid canary**: `.180` Mostafa stack, armed (`mofid_run_time=08:30`, `draft_count=10`), scheduler active. First 08:44 attempt was KILLED by the timeout bug (#180, now fixed); the NEXT open should fire **`IRO1DPAK0001`** — watch it.

### Open follow-ups (Session 42)
| # | Title | Why |
|---|---|---|
| — | **Watch the 08:44 open** for the Mofid canary | Confirm a clean bounded draft+batch BUY of `IRO1DPAK0001` at the RLC ceiling, the side=1 fire-log, date-based reconcile into `/admin/bot-report`. First real Mofid fire. |
